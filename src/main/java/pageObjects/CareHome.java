...
public class CareHome extends Base{
	...
	public void isPageDisplayed() {
		try {
			if(!careHomeQuestion.isDisplayed())
				throw new TestException("Care Home page is not displayed");
	        if(getDriver().getTitle().startsWith("Do you live permanently in a care home?")) {
		      
	          }
	        else {
		      throw new TestException("Error occured while loading Care Home page");
	          }
		}
		catch(Exception e) {
			throw new TestException("Care Home page is not displayed"+e.getMessage());
		}
	}
